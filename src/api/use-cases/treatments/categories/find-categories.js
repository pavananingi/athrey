    ...
    return Object.freeze({
        execute: async () => {
            try {
                ...
                if (!urlParams.uid) {
                    const result = (await treatmentCategoriesTable
                        .findAll({
                    ...
                    let result = (await treatmentCategoriesTable
                        .findByUID(urlParams.uid))
                        .data.categories;

                    if (result === null) {
                        throw new CreateError(translate(lang, 'category_not_found'), 404)
                    }
                ...
                logger.error(`Failed to find categories: %s`, error);
